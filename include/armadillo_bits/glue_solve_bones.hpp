...
class glue_solve_gen
  ...
  template<typename eT> inline static typename enable_if2<is_cx<eT>::no,  bool>::result guess_sympd(const Mat<eT>& A);
  template<typename eT> inline static typename enable_if2<is_cx<eT>::yes, bool>::result guess_sympd(const Mat<eT>& A);
  };
