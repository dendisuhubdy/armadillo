...
class SpProxy< SpOp<T1, spop_vectorise_col> >
  {
  public:
  ...
  typedef typename T1::elem_type                   eT;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef typename SpProxy<T1>::stored_type        stored_type;
  
  typedef typename SpMat<eT>::const_iterator       const_iterator_type;
  typedef typename SpMat<eT>::const_row_iterator   const_row_iterator_type;
  
  static const bool use_iterator   = SpProxy<T1>::use_iterator;
  static const bool Q_is_generated = SpProxy<T1>::Q_is_generated;
  ...
  arma_inline uword get_n_nonzero() const { return Q.get_n_nonzero(); }
  
  arma_inline elem_type operator[](const uword i)                    const { return Q[i];           }
  arma_inline elem_type at        (const uword row, const uword col) const { return Q.at(row, col); }
  
  arma_inline const eT*    get_values()      const { return Q.get_values();      }
  arma_inline const uword* get_row_indices() const { return Q.get_row_indices(); }
  arma_inline const uword* get_col_ptrs()    const { return Q.get_col_ptrs();    }
  ...
  template<typename eT2>
  arma_inline bool is_alias(const SpMat<eT2>& A) const { return Q.is_alias(A); }
  };
